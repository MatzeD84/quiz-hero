...
Apache liefert `nosniff`, Referrer-Policy, Permissions-Policy, Einbettungsschutz und HSTS. Im Quellstand läuft die vollständige Script-/Ressourcen-CSP zunächst im Report-Only-Modus, weil die gültigen Inline-Skript-Hashes erst nach SEO-Generierung und Asset-Versionierung feststehen. Der Deploymentbuild prüft alle ausgelieferten HTML-Dateien auf Inline-Styles, ergänzt die SHA-256-Hashes und wandelt die Richtlinie anschließend in genau eine erzwungene CSP um. HSTS gilt nur für HTTPS und enthält weder `includeSubDomains` noch Preload.

Sicherheits- und Cache-Header werden auf Produktion sowie im Docker-Webserver über `QUIZ_HERO_SERVER_MANAGED_HEADERS=true` ausdrücklich der `.htaccess` zugeordnet und nicht mehr ein zweites Mal durch PHP gesetzt. Standalone-PHP-Server erhalten weiterhin sichere PHP-Standardheader. Der Ergebnisfortschritt verwendet ein natives `<progress>`-Element; ausgeliefertes HTML und JavaScript benötigen damit weder Style-Attribute noch dynamische Inline-Styles. Strukturierte JSON-LD-Daten bleiben durch ihre Build-Hashes erlaubt, ohne `unsafe-inline` für Skripte oder Styles freizugeben.
...
- Der lokale Apache liefert die Sicherheits- und Cache-Header bei dynamischen PHP-Antworten jeweils genau einmal. Ein nachgebildetes Deployment enthielt 15 unterschiedliche, vollständig in der CSP enthaltene SHA-256-Hashes sowie genau eine erzwungene CSP ohne Report-Only und ohne `unsafe-inline`.
- Chrome bestätigte unter dem isolierten Apache-Deploy-Artefakt die Rom-Landingpage, einen vollständigen Drei-Fragen-Testlauf und den nativen Ergebnisfortschritt ohne CSP-Meldungen oder Websitefehler.
